                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    ...
                                    <div class="page-title-right">
                                    <?php if(in_array('add_module', is_inarray())) { ?>
    ...
    <input type="hidden" name="passok" id="passok" value="ok">
                                    ...
                                    <h5 class="modal-title" id="myModalLabel" >AJOUTER UN MODULE</h5>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                        </div>
                                                        <div class="modal-body">
                                                            <div class="mb-3">
                                        <label class="form-label">Nom module</label>
                                        <input type="text" id="username" name="username" class="form-control" required placeholder="Inscrire le nom du module" />
                                                            </div>
                                                        </div>
    ...
    <input type="hidden" name="id_module" id="id_module" value="">
                                                        <div class="modal-header">
                                    <h5 class="modal-title" id="myModalLabel" >MODIFIER UN MODULE</h5>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                        </div>
                                                        <div class="modal-body">
                                                            <div class="mb-3">
                    ...
                    <?php echo form_close();?>
                        ...
                        <table id="datatable" class="table table-striped table-bordered dt-responsive  nowrap w-100">
                                                    ...
                                                    <tbody>
            <?php if($liste_module>0){ $cpk=0;
                foreach($liste_module as $row){
                    $cpk++;?>
                                    <tr>
                                        <td><?=$cpk;?></td>
                                        <td style="text-transform: uppercase;"><?=$row->designation_module;?></td>
                                        <td>
                                           <div >
        <?php if(in_array('edit_module', is_inarray())) { ?>
            <a href="Javascript:void(0)" data-bs-toggle="modal" data-bs-target="#EditMyModal"class="green" data-name="<?php echo $row->id_liste_module;?>" data-nom="<?php echo $row->designation_module;?>"><button class="btn btn-success waves-effect waves-light"><i class="bx bx-pencil font-size-16 align-middle "></i></button></a>&nbsp;
        <?php } 
        if(in_array('delete_module', is_inarray())) { ?>
       <a href="Javascript:void(0)"  class="red"  data-name="<?php echo $row->id_liste_module;?>"><button class="btn btn-danger waves-effect waves-light"><i class="bx bx-trash font-size-16 align-middle"></i></button></a>
          <?php } ?>
                                            </div>
                                        </td>
                                    </tr>
...
<script>
    $(document).ready(function(){
        ...
        $(".green").on('click',function() { 
         ...
         $("form#addmodform1").on(
          ...
          $.ajax({
                url: "<?php echo base_url();?>administration/configuration/add_module",
              ...
              $("#getcontent").load("<?php echo base_url("administration/configuration/get_module")?>");
         ...
         $("form#editmodform1").on(
          ...
          $.ajax({
                url: "<?php echo base_url();?>administration/configuration/edit_module",
                type: $this.attr('method'),
                data: $this.serialize(),
                dataType: 'json', // JSON
                success: function(json) {
    ...
    function DeleteForm(id){
        ...
        $.ajax({
           type: "POST",
           url: "<?php echo base_url("administration/configuration/delete_module");?>",
           data: "delok=ok&id="+id,
