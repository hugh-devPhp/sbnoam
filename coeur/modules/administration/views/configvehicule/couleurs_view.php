        ...
        <div class="card">
            <div class="card-body">
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="myModalLabel" >AJOUTER UNE COULEUR</h5>
                        ...
                        <div id="EditMyModal" class="modal fade" role="dialog" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="myModalLabel" >MODIFIER UNE COULEUR</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <div class="mb-3">
                ...
                <table id="datatable" class="table table-striped table-bordered dt-responsive  nowrap w-100">
                    ...
                    <tbody>
                    <?php if(isset($couleurs) && !empty($couleurs)){ $cpk=0;
                        foreach($couleurs as $row){
                            $cpk++;?>
                            <tr>
                                <td><?=$cpk;?></td>
                                <td style="text-transform: uppercase;"><?=$row['code_couleur'];?></td>
                                <td style="text-transform: uppercase;"><?php if($row['activation_couleur']==1) echo "Active"; else echo "Inactive";?></td>
                                <td>
                                    <div >
                                        <?php if(in_array('edit_statut_couleur', is_inarray())) { ?>
                                            <a href="Javascript:void(0)"  class="blues" data-name="<?php echo $row['id_couleur'];?>" data-act="<?php echo $row['activation_couleur'];?>">
                                                <?php if($row['activation_couleur']==1){?><button class="btn btn-warning waves-effect waves-light"><i class="bx bx-power-off font-size-16 align-middle "></i></button><?php } else {?><button class="btn btn-primary waves-effect waves-light"><i class="bx bxs-torch font-size-16 align-middle "></i></button><?php } ?></a>&nbsp;
                                        <?php }
                                        if(in_array('edit_couleur', is_inarray())) { ?><a href="Javascript:void(0)" data-bs-toggle="modal" data-bs-target="#EditMyModal" class="green" data-name="<?php echo $row['id_couleur'];?>" data-nom="<?php echo $row['code_couleur']; ?>"  data-act="<?php echo $row['activation_couleur'];?>" ><button class="btn btn-success waves-effect waves-light"><i class="bx bx-pencil font-size-16 align-middle "></i></button></a>&nbsp;
                                        <?php }
                                        if(in_array('delete_couleur', is_inarray())) { ?>
                                            <a href="Javascript:void(0)"  class="red"  data-name="<?php echo $row['id_couleur'];?>"><button class="btn btn-danger waves-effect waves-light"><i class="bx bx-trash font-size-16 align-middle"></i></button></a>
                                        <?php } ?>
                                    </div>
                                </td>
                            </tr>
...
<script>
    $(document).ready(function(){
        ...
        $("form#addmodform1").on(
            ...
            function(e)
            {
                ...
                $.ajax({
                    url: "<?php echo base_url();?>administration/configvehicule/add_couleur",
                    ...
                    success: function(json) {
                        ...
                        if(json.reponse === '1') {
                            ...
                            $("#getcontent").load("<?php echo base_url("administration/configvehicule/get_couleur")?>");
        ...
        $("form#editmodform1").on(
            ...
            function()
            {
                ...
                $.ajax({
                    url: "<?php echo base_url();?>administration/configvehicule/edit_couleur",
                    type: $this.attr('method'),
                    data: $this.serialize(),
                    dataType: 'json', // JSON
                    success: function(json) {
    ...
    function DeleteForm(id){
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo base_url("administration/configvehicule/delete_couleur");?>",
            data: "delok=ok&id="+id,
            dataType: 'json', // JSON
            success: function(json){
                if(json.reponse=='1'){
